...
// This file implements a minimal datastore transaction server
...
import {
  server as WebSocketServer, connection
} from 'websocket';
...
wsServer.on('request', (request) => {
  ...
  if (!originIsAllowed(request.origin)) {
    ...
    request.reject();
    console.info((new Date()) + ' Connection from origin ' + request.origin + ' rejected.');
    return;
  }
  ...
  connection.on('message', (message) => {
    ...
    case 'storeid-request':
      ...
      reply = WSAdapterMessages.createStoreIdReplyMessage(data.msgId, idCounter++);
      break;
    case 'transaction-broadcast':
      ...
      const propMsgStr = JSON.stringify(WSAdapterMessages.createTransactionBroadcastMessage(
        propagate
      ));
    ...
    case 'history-request':
      ...
      reply = WSAdapterMessages.createHistoryReplyMessage(data.msgId, { transactions: history });
      break;
    default:
      return;
    }
  ...
  connection.on('close', (reasonCode, description) => {
    ...
    console.debug((new Date()) + ` Store ID ${storeId} disconnected. Reason: ${reasonCode}: ${description}`);
    for (let id in connections) {
      if (connections[id] === connection) {
