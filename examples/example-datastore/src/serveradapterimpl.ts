...
namespace WSAdapterMessages {
  ...
  type IBaseMessage = {
    ...
    msgType: (
      ...
      'fetch-patches-request' | 'fetch-patches-reply'
    );
    parentId: undefined;
  ...
  function createPatchFetchRequestMessage(patchIds: ReadonlyArray<string>): IPatchFetchRequestMessage {
    return createMessage('fetch-patches-request', { patchIds}) as IPatchFetchRequestMessage;
  }
...
class WSServerAdapter implements IServerAdapter, IDisposable {
  ...
  dispose(): void {
    ...
    this._delegates.forEach(delegate => {
      delegate.reject('Disposing adaptor');
    });
  ...
  private _onWSMessage(evt: MessageEvent) {
    ...
    if (msg.msgType === 'patch-broadcast') {
      ...
      this.broadcastLocal(msg.content.patch as Patch);
      handled = true;
    }
    if (!handled) {
