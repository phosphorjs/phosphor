...
/**
 * A layout which provides a flexible dashboard arrangement.
 *
 * #### Notes
 * The consumer of this layout is repsonsible for handling all signals
 * and managing the visibility of widgets as needed.
 */
export
class DashboardLayout extends Layout {
  ...
  /**
   * Set the inter-element spacing for the dashboard layout.
   */
  ...
  /**
   * Whether the dashboard layout is empty.
   */
  get isEmpty(): boolean {
  ...
  /**
   * Save the current configuration of the dashboard layout.
   *
   ...
   */
  saveLayout(): DashboardLayout.ILayoutConfig {
   ...
   */
  restoreLayout(config: DashboardLayout.ILayoutConfig): void {
    ...
    let mainConfig: DashboardLayout.AreaConfig | null;
    if (config.main) {
   ...
   *
   * @param widget - The widget to add to the dashboard layout.
   *
   ...
   */
  addWidget(widget: Widget, options: DashboardLayout.IAddOptions = {}): void {
    // Parse the options.
    let ref = options.ref || null;
    let mode = options.mode || 'split-left';

    // Find the single node which holds the reference widget.
    let refNode: Private.SingleLayoutNode | null = null;
    if (this._root && ref) {
      refNode = this._root.findSingleNode(ref);
    }
  ...
  /**
   * Find the single area which contains the given client position.
   *
   * @param clientX - The client X position of interest.
   *
   * @param clientY - The client Y position of interest.
   *
   * @returns The geometry of the single area at the given position, or
   *   `null` if there is no single area at the given position.
   */
  hitTestSingleAreas(clientX: number, clientY: number): DashboardLayout.ISingleAreaGeometry | null {
    ...
    // Find the single layout node at the local position.
    let singleNode = this._root.hitTestSingleNodes(x, y);

    // Bail if a single layout node was not found.
    if (!singleNode) {
      return null;
    }

    // Extract the data from the single node.
    let { widget, top, left, width, height } = singleNode;

    // Compute the right and bottom edges of the single area.
  ...
  private _removeWidget(widget: Widget): void {
    ...
    // Find the single node which contains the given widget.
    let singleNode = this._root.findSingleNode(widget);

    // Bail early if the single node is not found.
    if (!singleNode) {
      return;
    }

    // Handle the case where the single node is the root.
    if (this._root === singleNode) {
      this._root = null;
      return;
    }

    // Otherwise, remove the single node from its parent...

    // Prevent widget resizing unless needed.
    this._root.holdAllSizes();

    // Clear the parent reference on the single node.
    let splitNode = singleNode.parent!;
    ...
    // Handle the case where the child node is a single node.
    if (childNode instanceof Private.SingleLayoutNode) {
   ...
   */
  private _insertSplit(widget: Widget, ref: Widget | null, refNode: Private.SingleLayoutNode | null, orientation: Private.Orientation, after: boolean): void {
    ...
    // Create the single layout node to hold the widget.
    let singleNode = new Private.SingleLayoutNode(widget);
    ...
    if (!refNode || !refNode.parent) {
      ...
      // Determine the insert index for the new single node.
      let i = after ? root.children.length : 0;

      // Normalize the split node.
      root.normalizeSizes();

      // Create the sizer for new single node.
      let sizer = Private.createSizer(refNode ? 1 : Private.GOLDEN_RATIO);

      // Insert the single node sized to the golden ratio.
      ArrayExt.insert(root.children, i, singleNode);
      ArrayExt.insert(root.sizers, i, sizer);
      ArrayExt.insert(root.handles, i, this._createHandle());
      singleNode.parent = root;
    ...
    if (splitNode.orientation === orientation) {
      ...
      // Insert the single node sized to the other half.
      let j = i + (after ? 1 : 0);
      ArrayExt.insert(splitNode.children, j, singleNode);
      ArrayExt.insert(splitNode.sizers, j, Private.createSizer(s));
      ArrayExt.insert(splitNode.handles, j, this._createHandle());
      singleNode.parent = splitNode;
    ...
    // Add the single node sized to the other half.
    let j = after ? 1 : 0;
    ArrayExt.insert(childNode.children, j, singleNode);
    ArrayExt.insert(childNode.sizers, j, Private.createSizer(0.5));
    ArrayExt.insert(childNode.handles, j, this._createHandle());
    singleNode.parent = childNode;
...
export namespace DashboardLayout {
  ...
  interface IOptions {
    /**
     * The renderer to use for the dashboard layout.
     */
    renderer: IRenderer;
  ...
  /**
   * A renderer for use with a dashboard layout.
   */
  export
  interface IRenderer {
    /**
     * Create a new handle node for use with a dashboard layout.
     *
     * @returns A new handle node for a dashboard layout.
     */
   ...
   * to the dashboard layout relative to a reference widget.
   */
  export
  type InsertMode = (
    /**
     ...
     * inserted at the top edge of the dashboard layout.
     */
    'split-top' |
     ...
     *
     * If the reference widget is null or invalid, the widget will be
     * inserted at the left edge of the dashboard layout.
     */
    'split-left' |
     ...
     *
     * If the reference widget is null or invalid, the widget will be
     * inserted  at the right edge of the dashboard layout.
     */
    'split-right' |
     ...
     *
     * If the reference widget is null or invalid, the widget will be
     * inserted at the bottom edge of the dashboard layout.
     */
    'split-bottom'
  );

  /**
   * An options object for adding a widget to the dashboard layout.
   */
  export
  ...
  /**
   * A layout config object for a single widget area.
   */
  export
  interface ISingleAreaConfig {
    /**
   ...
   */
  export
  interface ISingleAreaGeometry {
    /**
     * The widget within this single area.
     */
    widget: Widget;
    ...
    /**
     * The local coordinate of the top edge of the single area.
     *
     * #### Notes
     * This is the distance from the top edge of the layout parent
     * widget, to the top edge of the single area.
     */
    top: number;

    /**
     * The local coordinate of the left edge of the single area.
     *
     * #### Notes
     * This is the distance from the left edge of the layout parent
     * widget, to the left edge of the single area.
     */
    left: number;

    /**
     * The local coordinate of the right edge of the single area.
     *
     * #### Notes
     * This is the distance from the right edge of the layout parent
     * widget, to the right edge of the single area.
     */
    right: number;

    /**
     * The local coordinate of the bottom edge of the single area.
     *
     * #### Notes
     * This is the distance from the bottom edge of the layout parent
     * widget, to the bottom edge of the single area.
     */
    bottom: number;

    /**
     * The width of the single area.
     *
     * #### Notes
     * This is total width allocated for the single area.
     */
    width: number;

    /**
     * The height of the single area.
     *
     * #### Notes
     * This is total height allocated for the single area.
     */
    height: number;
  }
}
...
namespace Private {
   ...
   */
  export
  type LayoutNode = SingleLayoutNode | SplitLayoutNode;
   ...
   */
  export
  function normalizeAreaConfig(config: DashboardLayout.AreaConfig, widgetSet: Set<Widget>): DashboardLayout.AreaConfig | null {
    let result: DashboardLayout.AreaConfig | null;
    if (config.type === 'single-area') {
      result = normalizeSingleAreaConfig(config, widgetSet);
    } else {
   ...
   */
  export
  function realizeAreaConfig(config: DashboardLayout.AreaConfig, renderer: DashboardLayout.IRenderer): LayoutNode {
    let node: LayoutNode;
    if (config.type === 'single-area') {
      node = realizeSingleAreaConfig(config, renderer);
    } else {
  ...
  /**
   * A layout node which holds the data for a single widget.
   */
  export
  class SingleLayoutNode {
    ...
    /**
     * Find the single layout node which contains the given widget.
     */
    findSingleNode(widget: Widget): SingleLayoutNode | null {
      return widget === this._widget ? this : null;
    }
     ...
     *
     * This ignores the sizers of single layout nodes.
     */
    holdAllSizes(): void {
      return;
    }
    ...
    fit(spacing: number, items: ItemMap): ElementExt.ISizeLimits {
      ...
      // Get the widget layout item.
      let widgetItem = items.get(this._widget);
      ...
      if (widgetItem && !widgetItem.isHidden) {
        ...
        this.sizer.minSize = widgetItem.minHeight;
        this.sizer.maxSize = Infinity;
      } else {
        this.sizer.minSize = 0;
        this.sizer.maxSize = Infinity;
      }
    ...
    update(left: number, top: number, width: number, height: number, spacing: number, items: ItemMap): void {
      ...
      BoxEngine.calc([this.sizer], height);

      // Layout the widget using the computed size.
      if (widgetItem && !widgetItem.isHidden) {
        let size = this.sizer.size;
  ...
  class SplitLayoutNode {
    ...
    findSingleNode(widget: Widget): SingleLayoutNode | null {
      for (let i = 0, n = this.children.length; i < n; ++i) {
        let result = this.children[i].findSingleNode(widget);
        if (result) {
          return result;
        }
      }
    ...
    hitTestSingleNodes(x: number, y: number): SingleLayoutNode | null {
      for (let i = 0, n = this.children.length; i < n; ++i) {
        let result = this.children[i].hitTestSingleNodes(x, y);
        if (result) {
          return result;
        }
      }
     ...
     * Create a configuration object for the layout tree.
     */
    createConfig(): DashboardLayout.ISplitAreaConfig {
  ...
  /**
   * Normalize a single area config and collect the visited widgets.
   */
  function normalizeSingleAreaConfig(config: DashboardLayout.ISingleAreaConfig, widgetSet: Set<Widget>): DashboardLayout.ISingleAreaConfig | null {
   ...
   */
  function normalizeSplitAreaConfig(config: DashboardLayout.ISplitAreaConfig, widgetSet: Set<Widget>): DashboardLayout.AreaConfig | null {
    // Set up the result variables.
    let orientation = config.orientation;
    let children: DashboardLayout.AreaConfig[] = [];
    let sizes: number[] = [];
    ...
    for (let i = 0, n = config.children.length; i < n; ++i) {
      ...
      if (child.type === 'single-area' || child.orientation !== orientation) {
        children.push(child);
  ...
  /**
   * Convert a normalized single area config into a layout tree.
   */
