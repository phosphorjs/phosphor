...
import {
  formatKeystroke, keystrokeForKeydownEvent, normalizeKeystroke
} from './keystroke';
...
class Keymap {
  ...
  findBinding(command: string, args: JSONObject | null): Keymap.IBinding | undefined {
    let i = findLastIndex(this._bindings, binding => {
      return binding.command === command && deepEqual(binding.args, args);
    });
    return i !== -1 ? this._bindings[i] : undefined;
  }
  ...
  addBinding(options: Keymap.IBindingOptions): IDisposable {
    ...
    return new DisposableDelegate(() => {
      ...
      let i = this._bindings.indexOf(binding);
      if (i !== -1) {
